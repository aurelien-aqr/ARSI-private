...
cases = [c for c in doc["cases"] if c["reference"].startswith("3333")]
m.check_model(MODEL)
print(f"{len(cases)} cas 3333, {sum(len(c.get('instances') or []) for c in cases)} instances\n",
      flush=True)
...
for name, tile, margin, context in ARMS:
    ...
    print(f"{name:9} tile={tile or '-':>4} | {calls:5} appels {dt:6.0f}s | "
          ...
          flush=True)
    json.dump(results, open("/tmp/tile_sweep_result.json", "w"), indent=2)
